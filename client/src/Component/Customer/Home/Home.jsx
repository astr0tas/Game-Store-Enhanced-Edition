...
const Game = (props) =>
{
      ...
      return (
            <div className={ `card border border-dark h-100 p-0 my-3 ${ styles.cards }` }>
                  <img className='card-img-top w-100' alt='' src={ props.img === null ? 'https://upload.wikimedia.org/wikipedia/commons/7/71/Nothing_whitespace_blank.png' : `http://${ domain }/model/data/games/${ props.img }` }></img>
      ...
      useEffect(() =>
      {
            ...
            axios.get(`http://${ domain }/getBestSeller`)
                  .then(res =>
                  {
                        ...
                        {
                              if (i < 3)
                                    temp1.push(<Game key={ i } id={ res.data[i].id } name={ res.data[i].name } img={ res.data[i].picture_1 } price={ res.data[i].price } discount={ res.data[i].discount } />);
                              else
      ...
      return (
            <div className='w-100 h-100 d-flex flex-column align-items-center' ref={ popUpContainer }>
                  ...
                  <div className='flex-grow-1 overflow-auto w-100 row'>
                        <div className='col-12 d-flex container-fluid justify-content-lg-around flex-column align-items-center flex-lg-row align-items-lg-start my-auto' ref={ div1 }>
                        </div>
                        <div className='col-12 d-flex container-fluid justify-content-lg-around flex-column align-items-center flex-lg-row align-items-lg-start my-auto' ref={ div2 }>
                        </div>
                  </div>
